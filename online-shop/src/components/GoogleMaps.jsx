...
const GoogleMaps = (props) => {
  ...
  let [infovisible, setInfoVisible] = useState(true);
  const [nodDeliveryData, setnodDeliveryData] = useState(true);
  const firestore = new firestoredb();
  const [haslatlongdata, setHasLatLongData] = useState(false);
 ...
 useEffect(() => {
    ...
    if (latitude === 0 && longitude === 0) {
      ...
      if (rerendercount >= 1) {setOpen(true);}
      // setZoom(18)
    }
 }, [latitude, longitude]);
