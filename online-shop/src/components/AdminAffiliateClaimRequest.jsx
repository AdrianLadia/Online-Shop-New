...
import { TextField, Table, TableHead, TableBody, TableRow, TableCell, TableContainer, Paper, Button, Modal, Box, Divider } from '@mui/material';
...
const AffiliateClaimRequest = () => {
  ...
  function onUploadImage(imageUrl) { setImageProof(imageUrl) }
  ...
  return (
    <div className='flex flex-col justify-center items-center py-5 gap-5'>
      ...
      <div className=' p-5 rounded-lg text-color30 text-3xl tracking-widest shadow-md border-t border-x border-color30 shadow-color30'>Affiliate Claim Requests</div>
      <div className='w-19/20 md:w-11/12'>
        <TableContainer className="border-2 border-color30 overflow-auto" component={Paper} sx={{ maxHeight: height }}>
            ...
            <TableBody >
              {transactionData != 0 ? transactionData.map((data, index)=>(
                <TableRow key={index}>
                  ...
                  <TableCell style={{color:data.amount <= data.totalDeposited?"mediumseagreen":"red"}}>₱{data.totalDeposited.toLocaleString()}</TableCell>
                  <TableCell>
        ...
        <Modal open={open} onClose={()=>handleClose()} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
          <Box sx={style} className="max-h-9/10 sm:max-h-7/10 overflow-auto rounded-xl border-color30 bg-green4">
            ...
            <div className='flex justify-center items-center w-full p-5 mt-5 border border-color30 rounded-lg'>
                {imageProof?<img className='h-full' alt='Upload' src={imageProof}/>:<FaRegImage className=' text-color30 h-28 w-24'/>}
            </div>
            <div className='mt-5'>
              <ImageUploadButton buttonTitle="Upload Proof" onUploadFunction={onUploadImage} storage={storage} folderName={`affiliateDeposits/${affiliateUserId}/`}/>
            </div>
            <Divider className='my-6'/>
            <div className='flex justify-center items-center w-full'>
              <TextField color='primary' type='number' onChange={(e)=>setAmountDeposited(e.target.value)} label='Amount'/>
            </div>
            <Divider className='mt-6'/>
            <div className='flex justify-center items-center w-full p-5'>
              <Button disabled={imageProof && amountDeposited?false:true} variant='contained' className='rounded-md p-3 px-6' onClick={()=>handleModalDepositClick()}>Deposit</Button>
            </div>
          </Box>
        </Modal>
      </div>
