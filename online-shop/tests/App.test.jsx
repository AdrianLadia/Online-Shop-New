...
test('getSafetyStock', async ({ assert }) => {
  

  const validResult = businesscalculations.getSafetyStock(10);
  assert.is(validResult, 20, 'Valid input should return correct safety stock');

  try {
    businesscalculations.getSafetyStock('invalid input');
    assert.unreachable('Invalid input should throw an error');
  } catch (error) {
    assert.ok(error instanceof Error, 'Error should be thrown for invalid input');
  }
});

test('getStocksAvailableLessSafetyStock', async ({ assert }) => {
  

  const validResult = businesscalculations.getStocksAvailableLessSafetyStock(50, 10);
  assert.is(validResult, 30, 'Valid input should return correct stocks available less safety stock');

  try {
    businesscalculations.getStocksAvailableLessSafetyStock('invalid input', 10);
    assert.unreachable('Invalid input should throw an error');
  } catch (error) {
    assert.ok(error instanceof Error, 'Error should be thrown for invalid input');
  }
...
test('getCartCount', async ({ assert }) => {
  

  const cart = ['item1', 'item2', 'item1', 'item3'];
  const validResult = businesscalculations.getCartCount(cart);
  assert.deepEqual(validResult, { item1: 2, item2: 1, item3: 1 }, 'Valid input should return correct cart count');

  try {
    businesscalculations.getCartCount('invalid input');
    assert.unreachable('Invalid input should throw an error');
  } catch (error) {
    assert.ok(error instanceof Error, 'Error should be thrown for invalid input');
  }
