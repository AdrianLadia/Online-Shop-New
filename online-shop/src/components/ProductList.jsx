...
const ProductList = () => {
  ...
  function RenderSelectedProducts(product_category) {
    ...
    for (let i = 0; i < products.length; i++) {
      if (products[i].category === product_category) {
  ...
  return (
    <div>
      <div className={"flex flex-col justify-center items-center  " + divCssIfProductNoteLoaded() }>
        ...
        ) : RenderSelectedProducts(selected).length === 0 &&
          selected === "Favorites" ? (
          <div>
            ...
            <h3 className="text-1xl font-bold text-center mt-10 mx-10">
              Add items to your favorites by clicking on the product card then
            ...
            return (
              <div>
                <ProductCard
                  addtocart={AddToCart}
                  product={product}
                  key={'productCard-'+product.itemid}
                />
              </div>
            );
          })
