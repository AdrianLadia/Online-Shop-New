...
const App = () => {
  ...
  useEffect(() => {
    if (rules.checkIfUserAuthorized('itemAnalytics') === true) {
          ...
          <div className=" overflow-hidden bg-gradient-to-r h-screen from-green-200 via-cyan-200 to-emerald-200">
            {screenSizeMobile === true ? (
              <div className="flex justify-start absolute top-13 left-1 ml-0 xs:ml-4 ">
                <button
                  className=" mt-2 mr-2 p-3 text-3xl 2xs:text-4xl rounded-lg text-green-700 hover:text-emerald-500 "
            ...
            <div className="h-5/6 w-screen flex mt-16 justify-center">
              <>
                <>{screenSizeMobile === false ? <MenuBar callback={handleTableContent} products={products} /> : null}</>
              </>
              <>
                {showMenu === false ? (
                  <>
